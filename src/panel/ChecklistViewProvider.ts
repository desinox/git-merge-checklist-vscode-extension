...
import {
	openCommitFileDiff,
	openDiffWithWorkingTree,
} from "../diff/CommitDiffProvider";
...
export class ChecklistViewProvider implements vscode.WebviewViewProvider {
		...
		try {
			switch (msg.type) {
				...
				case "getCommitDetails": {
					const details = await this.git.getCommitDetails(msg.hash);
					this.post({
						type: "commitDetails",
						hash: msg.hash,
						details,
	...
	private async sendInit(): Promise<void> {
		if (!(await this.git.isRepo())) {
			this.post({
				type: "error",
				message: "No git repository found in the workspace.",
			});
			return;
		}
	...
	private async cherryPick(hash: string, subject: string): Promise<void> {
		const items: {
			...
			},
			{
				...
				description:
					"Apply, then edit files/message and commit from Source Control",
				mode: "edit",
			},
			{
				label: "$(git-commit) Cherry Pick without Commit",
				description:
					"Apply only (uncommitted); the original author is kept when committing",
				mode: "no-commit",
			},
		];
	...
	private async handleCherryPickError(err: unknown): Promise<void> {
		const message = err instanceof Error ? err.message : String(err);
		const isConflict =
			/conflict|after resolving|could not apply|fix conflicts/i.test(
				message,
			);
		if (isConflict) {
